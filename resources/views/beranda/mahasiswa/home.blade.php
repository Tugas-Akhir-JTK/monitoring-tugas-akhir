        ...
        <div class="row mb-2">
          ...
          @if (auth()->user()->role == '1' || auth()->user()->role == '3')
            <div class="col d-grid d-md-flex justify-content-md-end gap-2">
              <span class="badge badge-pill badge-success d-flex justify-content-center align-items-center mr-1"
                style="font-size: 1.0em;">
                <i class="nav-icon fas fa-check mr-1"></i>
                <span>Sudah Mengumpulkan</span>
              </span>
              <span class="badge badge-pill badge-secondary d-flex justify-content-center align-items-center"
                style="font-size: 1.0em;">
                <i class="nav-icon fas fa-times mr-1"></i>
          ...
          <div class="row">
            ...
            <div class="col-5 d-md-flex justify-content-md-end">
              <div class="mr-2">
                <span class="badge bg-light text-dark me-3">
                  Jumlah Bimbingan : {{ $resume_bimbingan_1 }}
                </span>
              </div>
              @if (auth()->user()->role == '2')
              ...
              {{-- @foreach ($tahapan_progres as $item)
                @if ($item->id_master_tahapan_progres == 1)
                  <form action="{{ route('kota.status') }}" method="POST" id="statusForm_{{ $item->id }}">
                    ...
                    <input type="hidden" name="id_master_tahapan_progres"
                      value="{{ $item->id_master_tahapan_progres }}">
                    <div class="form-group">
                      <select class="form-control-sm" id="statusControlSelect_{{ $item->id }}" name="status"
                        onchange="submitForm('{{ $item->id }}')">
                        <option value="selesai" class="badge badge-primary"
                          {{ $item->status == 'selesai' ? 'selected' : '' }}>Selesai</option>
                        <option value="on_progres" class="badge badge-warning"
                          {{ $item->status == 'on_progres' ? 'selected' : '' }}>On-Progres</option>
                      </select>
                    </div>
                  </form>
                @endif
            ...
            <div class="col justify-content-md-end">
              <div class="progress" style="height: 25px;">
                <div class="progress-bar" role="progressbar" style="width: {{ number_format($selesaiPercentage1, 1) }}%;"
                  aria-valuenow="{{ number_format($selesaiPercentage1, 1) }}" aria-valuemin="0" aria-valuemax="100">
                  {{ number_format($selesaiPercentage1, 1) }}%</div>
              </div>
            </div>
          </div>
        </div>
              ...
              <div class="col mr-2">
                @if ($isSubmitted)
                  <a href="{{ route('home.showFile', $submittedFileId) }}">
                    <span class="badge badge-pill badge-success" style="padding : 7px 12px">
                      <i class="nav-icon fas fa-file mr-1"></i>
                      {{ $masterArtefak->nama_artefak }}
                    </span>
                  </a>
                @else
                  <span class="badge badge-pill badge-secondary" style="padding : 7px 12px">
                    <i class="nav-icon fas fa-file mr-1"></i>
                    {{ $masterArtefak->nama_artefak }}
                  </span>
                @endif
    ...
    <div class="alert" role="alert" style="background-color: #D2DCF2;">
      <div class="row">
        ...
        <div class="col-5 d-md-flex justify-content-md-end">
          <div class="mr-2">
            <span class="badge bg-light text-dark me-3">
              Jumlah Bimbingan : {{ $resume_bimbingan_2 }}
            </span>
          </div>
          @if (auth()->user()->role == '2')
            <div class="form-group">
        ...
        <div class="col justify-content-md-end">
          <div class="progress" style="height: 25px;">
            <div class="progress-bar" role="progressbar" style="width: {{ number_format($selesaiPercentage2, 1) }}%;"
              aria-valuenow="{{ number_format($selesaiPercentage2, 1) }}" aria-valuemin="0" aria-valuemax="100">
              {{ number_format($selesaiPercentage2, 1) }}%</div>
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="alert" role="alert" style="background-color: #D2DCF2;">
      <div class="row">
        ...
        <div class="col-5 d-md-flex justify-content-md-end">
          <div class="mr-2">
            <span class="badge bg-light text-dark me-3">
              Jumlah Bimbingan : {{ $resume_bimbingan_3 }}
            </span>
          </div>
          @if (auth()->user()->role == '2')
            <div class="form-group">
        ...
        <div class="col justify-content-md-end">
          <div class="progress" style="height: 25px;">
            <div class="progress-bar" role="progressbar" style="width: {{ number_format($selesaiPercentage3, 1) }}%;"
              aria-valuenow="{{ number_format($selesaiPercentage3, 1) }}" aria-valuemin="0" aria-valuemax="100">
              {{ number_format($selesaiPercentage3, 1) }}%</div>
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="alert" role="alert" style="background-color: #D2DCF2;">
      <div class="row">
        ...
        <div class="col-5 d-md-flex justify-content-md-end">
          <div class="mr-2">
            <span class="badge bg-light text-dark me-3">
              Jumlah Bimbingan : {{ $resume_bimbingan_4 }}
            </span>
          </div>
          @if (auth()->user()->role == '2')
            <div class="form-group">
        ...
        <div class="col justify-content-md-end">
          <div class="progress" style="height: 25px;">
            <div class="progress-bar" role="progressbar" style="width: {{ number_format($selesaiPercentage4, 1) }}%;"
              aria-valuenow="{{ number_format($selesaiPercentage4, 1) }}%;" aria-valuemin="0" aria-valuemax="100">
              {{ number_format($selesaiPercentage4, 1) }}%</div>
          </div>
        </div>
      </div>
    </div>
